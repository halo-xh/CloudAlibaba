...
import com.m3u8.Exception.M3u8Exception;
import com.m3u8.listener.DownloadListener;
import com.m3u8.utils.Constant;
import com.m3u8.utils.MediaFormat;
import com.m3u8.utils.M3u8StringUtils;
...
import static com.m3u8.utils.Constant.FILESEPARATOR;
...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        private void startDownload() {
            ...
            for (String s : tsSet) {
                i++;
                fixedThreadPool.execute(createDownloadThread(s, i));
            }
            ...
            new Thread(() -> {
                ...
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        Thread.sleep(1000L);
                        log.info("已用时" + consume + "秒！\t下载速度：" + M3u8StringUtils.convertToDownloadSpeed(new BigDecimal(downloadBytes.toString()).subtract(bigDecimal), 3) + "/s");
                ...
                log.info("下载完成，正在合并文件！共" + finishedFiles.size() + "个！" + M3u8StringUtils.convertToDownloadSpeed(downloadBytes, 3));
                //开始合并视频
                LocalDateTime start = LocalDateTime.now();
                mergeTs();
                //删除多余的ts片段
                deleteFiles();
                System.out.println("耗时 = " + Duration.between(start, LocalDateTime.now()).toMillis());
            ...
            new Thread(() -> {
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        Thread.sleep(1000L);
                        for (DownloadListener downloadListener : listenerSet)
                            downloadListener.speed(M3u8StringUtils.convertToDownloadSpeed(new BigDecimal(downloadBytes.toString()).subtract(bigDecimal), 3) + "/s");
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                }
         ...
         * @return 线程
         */
        private Thread createDownloadThread(String urls, int i) {
            return new Thread(() -> {
                int count = 1;
                ...
                while (count <= retryCount) {
                    try {
                        ...
                        log.error("第" + count + "获取链接重试！\t" + urls, e);
                        count++;
                    } finally {
                        try {
        ...
        private String getTsUrl() {
            ...
            if (!contentVal.contains("#EXTM3U"))
                throw new M3u8Exception(DOWNLOADURL + "不是m3u8链接！");
            String[] split = contentVal.split("\\n");
            String keyUrl = "";
            boolean isKey = false;
            for (String s : split) {
        ...
        private String getKey(String url, StringBuilder content) {
            ...
            for (int i = 0; i < split.length; i++) {
                ...
                if (s.contains("#EXTINF")) {
                    String s1 = split[++i];
                    tsSet.add(M3u8StringUtils.isUrl(s1) ? s1 : mergeUrl(relativeUrl, s1));
                }
            }
            if (StringUtils.hasText(key)) {
                key = key.replace("\"", "");
                return getUrlContent(M3u8StringUtils.isUrl(key) ? key : mergeUrl(relativeUrl, key), true).toString().replaceAll("\\s+", "");
            }
            return null;
        }
...
//                    e.printStackTrace();
        ...
        private byte[] decrypt(byte[] sSrc, int length, String sKey, String iv, String method) throws Exception {
            if (StringUtils.hasText(method) && !method.contains("AES"))
            ...
            byte[] ivByte;
            if (iv.startsWith("0x"))
                ivByte = M3u8StringUtils.hexStringToByteArray(iv.substring(2));
